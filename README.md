> [!NOTE]  
> 17-09-2025 - (ANOTHER NOTES) I'm probably wrong about LLM right now. But considering what I'd like to achieve here (my main goal) is to have it build graphs independently and try to solve programming problems, i.e., writing code, verifying errors, and fixing bugs. This is the "simplest" thing you can try to build as a benchmark, because it provides a lot of spaces for learning and testing. (...) On the other hand, when I think about it now, there's something fundamental that LeNet or AlexNet, for example, solved, and then the transformer did by adding multihead attention (is this the part that increases context and its diversity the most? I'm not sure now because I haven't checked it thoroughly yet). That is, the ability to find patterns in large amounts of similar data. A simple example is facial images (face recognition problem). There are many similar people, for example, models (female) who have similar features, eyes, and facial features. The LeNet and AlexNet fundamentally solve the problem of better recognition of these details, allowing us to determine who a given person in a photo is. Then there are other things related to these architectures, such as different sizes, profiles from which the photo was taken, etc. But fundamentally, LeNet allowed for generalization and better searches for these types of patterns. (...) Why am I writing about this? Leaving aside my silly challenge approach, these architectures allows for this type of classification. I probably won't touch on fundamental things like that in this repo. Because here there is a fundamental problem between what MLP does (i.e. predicting sequences, subsequent tokens) and what LLM does, i.e. a higher level of abstraction, i.e. operating on these data, tokens. It'll probably be another silly challenge for myself to learn something from it. A simple demo to explore the graph approach could be learning a model for better code debugging and bug fixing. But that's for the future... First, I need to move from the TODO phase to completing a task at the end of the day. But when I thought about it for a while, debugging code seems like a cool thing to learn because you can produce a lot of errors, etc. (...) STILL TODO


> [!NOTE]  
>  MLP that Andrej gave to learn, how to train network on text, that can pack and extract patterns from little database of names. It can learn patterns from 32,000 names and reproduce them. GPT-2 and Transformer look like they were trying to build a much larger model that does the same thing ( pack all the text from the internet ) + understands text. It looks like they tried to build a much larger model that learns patterns from text in the same way back in 2017. Now is GPT-5, is amazing. Amazing work by the engineering and all the other experts behind it. And the work ethic of OpenAI, the organization, the way it solves problems, etc. (...) Today, I can't work like them, I can't solve problems like them, be organized like them, etc. You can learn from people like them. So for now, I'll leave this topic with "tiny model." To do that, I need to learn better PyTorch documentation, python, and stuff like that to solve this. So it will take me a while to get back to managing this repo. IT IS CLOSED FOR NOW - 06-09-2025.
