...
python extract_and_build.py --input input.txt --pattern "if " --plain --out matches.txt // It builds "connections" according to the strategy if you confirm "y." But this isn't necessary for a quick test.
...
```
[plan] canonical tokens: 8, instances: 16, adjacency edges: 15
C:\Users\...\SVG-Mind-Tree-main\SVG-Mind-Tree-main\version_4\write_plan_
to_db.py:34: DeprecationWarning: datetime.datetime.utcnow() is deprecated and scheduled for removal
in a future version. Use timezone-aware objects to represent datetimes in UTC: datetime.datetime.now
(datetime.UTC).
  return datetime.utcnow().isoformat(timespec='seconds')
C:\Users\...\SVG-Mind-Tree-main\SVG-Mind-Tree-main\version_4\write_plan_
to_db.py:56: DeprecationWarning: datetime.datetime.utcnow() is deprecated and scheduled for removal
in a future version. Use timezone-aware objects to represent datetimes in UTC: datetime.datetime.now
(datetime.UTC).
  time_field = datetime.utcnow().strftime("%H:%M:%S")
[done] committed changes. Created canonical nodes: 8, created instance nodes: 16, created adjacency
connections: 15
```
